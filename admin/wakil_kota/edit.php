...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    $sql = "UPDATE wakil_kota SET masa_jabatan = ?, nama = ?, tgl = ?";
    if(!empty($masa) && !empty($nama)){
        if($stmt = mysqli_prepare($link, $sql)){
            mysqli_stmt_bind_param($stmt, 'sss', $masa, $nama, $tgl);
            if(mysqli_stmt_execute($stmt)){header("Location: daftar.php? edit Wali-Kota sukses");mysqli_stmt_close($stmt);mysqli_close($link);exit();}
...
<body>
    ...
    $result = mysqli_query($link, "SELECT * FROM wakil_kota WHERE id = '$id'");
    ...
    ?>
        <table align="center" valign="middle">
            ...
            <tr>
                ...
                <td><input type="text" name="nama" value="<?php echo $row[2];?>"></td>
            </tr>
            <tr>
